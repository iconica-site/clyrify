...
var swiper7 = new Swiper('.lichnaya-info-slider', {
    slidesPerView: 'auto', // чтобы не листалось по одному слайду надо здесь поставить авто и добавить freeMode true
...
let currentStep = 1;
    ...
    function nextStep() {
        ...
        currentStep++;
        if (currentStep > 7) {
            currentStep = 7; // Ограничиваем количество шагов
        }
        document.getElementById('step-' + currentStep).classList.add('active');
        updatePagination();
        updateButtonAndTotal();
        if (currentStep === 7) hidePagination(); // Скрыть пагинацию, если текущий шаг 7
    }
    ...
    function prevStep() {
        ...
        updatePagination();
        updateButtonAndTotal();
        if (currentStep !== 7) showPagination(); // Показать пагинацию, если шаг не 7
    }

    function updatePagination() {
        const indicators = document.querySelectorAll('.step-indicator');
        const stepText = document.querySelector('.pagination span'); // Для обновления текста шага
    
        indicators.forEach((indicator, index) => {
            if (index + 1 <= currentStep) {
                indicator.classList.add('active'); // Активируем все шаги до текущего
            } else {
                indicator.classList.remove('active'); // Убираем активность у следующих шагов
            }
        });
    
        // Обновляем текст шага, например, "1 of 4"
        stepText.textContent = `${currentStep} of ${indicators.length}`;
    }
    ...
    function updateButtonAndTotal() {
        ...
        if (currentStep === 4) {
            ...
            pagination.style.display = 'flex'; // Скрыть пагинацию
        } else {
            nextButton.style.display = 'flex'; // Показать кнопку "Далее"
    ...
    function hidePagination() {
        const pagination = document.querySelector('.step-header-pagination');
        const nextButton = document.getElementById('next-button');
        ...
        if (currentStep === 7) {
            ...
            pagination.style.display = 'flex'; // Показать пагинацию
            nextButton.style.display = 'flex'; // Скрыть кнопку "Далее"
        }
    }
